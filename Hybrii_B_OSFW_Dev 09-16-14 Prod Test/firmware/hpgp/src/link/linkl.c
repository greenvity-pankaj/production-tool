...
u8 LINKL_DetermineStaTrans(sLinkLayer *linkLayer, sEvent *event)
{
...
void LINKL_PreProcStaEvent(sLinkLayer *linkLayer, sEvent *event)
{
    ...
    event->eventHdr.trans = LINKL_DetermineStaTrans(linkLayer, event);
} 
...
u8 LINKL_DetermineCcoTrans(sLinkLayer *linkLayer, sEvent *event)
{
    u8 trans = LINKL_TRANS_UNKNOWN;
    if(event->eventHdr.eventClass == EVENT_CLASS_MSG)
    {
...
void LINKL_PreProcCcoEvent(sLinkLayer *linkLayer, sEvent *event)
{
    ...
    event->eventHdr.trans = LINKL_DetermineCcoTrans(linkLayer, event);
} 
...
eStatus LINKL_SetLineMode(sLinkLayer *linkLayer, eLineMode lineMd)
{
   ...
   HHAL_SetDevMode(hal, DEV_MODE_STA, lineMd);
...
void LINKL_SetStaMode(sLinkLayer *linkLayer)
{
    ...
    //HHAL_SetDevMode(linkLayer->hal, DEV_MODE_STA, LINE_MODE_DC);
#else
...
FM_Printf(FM_ERROR, "tei:%d, staCap %x\n", 
        ...
        for(j=0; j<MAXSNID; j++)
        {
            for(i=0; i<linkLayer->staNsm.numAvln; i++)
            {
                if(linkLayer->staNsm.avlnInfo[i].snid == j)
                {
                    break;
                }
            }
            if(i == linkLayer->staNsm.numAvln)
            {                
                staInfo->snid = j;                
                break;
            }
	...
	staInfo->ccoScb->staCap.fields.pcoStatus = 0;
		...
		HHAL_SetDevMode(linkLayer->hal, DEV_MODE_CCO, gHpgpHalCB.lineMode);
...
void LINKL_PostStopCCo(sLinkLayer *linkLayer)
{
	...
	HHAL_SetDevMode(linkLayer->hal, DEV_MODE_STA, 
				gHpgpHalCB.lineMode);
...
//    LINKL_InitCcoInfo(linkLayer);
  ...
  //  os_create_task(HPGP_TASK_ID_LINK);
    ...
    {
        HHAL_SetDevMode(linkLayer->hal, DEV_MODE_STA, gHpgpHalCB.lineMode);
    }
#else
    HAL_SetDevMode(linkLayer->hal, DEV_MODE_STA);
#endif
