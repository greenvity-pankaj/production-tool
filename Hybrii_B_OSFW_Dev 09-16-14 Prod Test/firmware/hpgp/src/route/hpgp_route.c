        ...
        {   
                ...
                ((pscb->lrtEntry.rnh == (rInfo->rnh + 1)) && (pscb->lrtEntry.rdr < rInfo->rdr)))
            {
                ...
                if(pscb->lrtEntry.routeIsInvalid == TRUE)
                {
                    ...
                    Host_SendIndication(HOST_EVENT_ROUTE_VALID, (u8*)&rEvent, sizeof(routeEvent));
                }
                else
                {
                    
                    Host_SendIndication(HOST_EVENT_ROUTE_CHANGE, (u8*)&rEvent, sizeof(routeEvent));
                }
            }
        }
        numEntris--;
...
eStatus ROUTE_sendUnreachableInd(u32 ntb)
{
    ...
    Host_SendIndication(HOST_EVENT_ROUTE_HOLD, &gRoute.holdlist, gRoute.numOfTeisOnHold);
    return status;
...
void ROUTE_procHdDurationTimeout()
{
    ...
    {                            
        if(pscb->lrtEntry.routeOnHold == TRUE)
        {
            ...
            Host_SendIndication(HOST_EVENT_ROUTE_INVALID, &pscb->tei, 1);
        }
        pscb = CRM_GetNextScb(crm, pscb);
    }
