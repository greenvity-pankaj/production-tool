 ...
 * MAC Entry point
 *
 * $Id: mac.c,v 1.7 2014/11/26 13:19:40 ranjan Exp $
 *
...
{
    printf("\nS/W TX Stats:\n");
    printf(" NoAck=%lu, NoCCA=%u, FrmPending=%lu, Errs=%u\n", 
...
{
    printf("\nS/W RX Stats:\n");
    printf("Pkts rxed=%lu - Bytes rxed=%lu\n",
	       mac_stats_g.rx_pkts_count, mac_stats_g.rx_bytes_count);
    printf("Decrypt Err=%u, Decrypt Ok=%u, No buffer=%u, "
           "Frm too big=%u, Bad CRC=%u\n",
...
static void mac_soft_reset (uint8_t init_pib)
{
    ...
    // FIXME - Add code to stop of running timer
#if 1
#ifdef RTX51_TINY_OS	
		STM_StopTimer(indirect_data_persistence_timer);
		...
		STM_StopTimer(mac_rsp_wait_timer);			
