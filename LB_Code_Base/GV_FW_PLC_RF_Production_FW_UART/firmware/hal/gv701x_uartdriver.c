...
#ifdef PROD_TOOL
extern bool isProdTstCmd(u8 *pCpAddr, u8 cpLen, sprodTstCmd *pprodCmdTst); 
extern void prodTestExecCmd(sprodTstCmd *pprodTestCmd);
#endif
...
void uart_handler(void) __INTERRUPT2__ using 2
{
...
volatile u8 uartRxBufferProdTest[APP_DATA_MAX_SIZE];
volatile u16 uartRxLengthProdTest;
void uartRxProc()
{
#ifndef NO_HOST
	eStatus status;
...
#else // NO_HOST
#ifdef PROD_TEST
			memcpy(&uartRxBufferProdTest[0],rxBuffer,uartRxControl.rxCount);
			uartRxLengthProdTest = uartRxControl.rxCount;

			//printf("Uart Rx Packet size %u\n",uartRxControl.rxCount);
			EA = 1;
			if(isProdTstCmd(uartRxBufferProdTest, uartRxLengthProdTest, &prodCmd))
			{
...
void hal_uart_buffer (uint8_t XDATA *tx_data_p, uint16_t tx_req_bytes)

{
	u8 intFlag=0;	
//	xdata uint8_t *cellAddrTx;
	intFlag = EA;
	//printf("hal_uart_buffer\n");
	//FM_HexDump(FM_USER,"hal_uart_buff",tx_data_p,tx_req_bytes);
	EA = 0;
	//memcpy((uint8_t*)&uartTxControl.uartTxFrmSwDesc,(uint8_t *)pHostTxFrmSwDesc,sizeof(sSwFrmDesc));
#ifdef LG_UART_CONFIG	
	if(uartTxControl.txModeControl == UART_TX_AUTO)
	...
	uart_fifoctrl->fifoctrl   = UART_FIFO_RX_1_TX_8;
#else
	uart_fifoctrl->fifoctrl   = UART_FIFO_RX_1_TX_8;//rx 8
#endif
	uartTxControl.pTxBuffer = txBuffer;
	uartTxControl.txCount = tx_req_bytes;
	uartTxControl.txDone = 0;
	memcpy(uartTxControl.pTxBuffer, tx_data_p, tx_req_bytes);

	//printf("Uart TX Len %u, intflag %bu\n",uartTxControl.txCount,intFlag);
	// uart interrupt disabled temporarily
	uart_intctrl->intctrl_field.EnTxEmptInt = 1; // enables tx interrupt
	//txQPending = 1;
	EA = 1;
	//return STATUS_SUCCESS;
