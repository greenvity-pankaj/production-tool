...
static uint_8t XDATA sbox[256]  =  sb_data(f1);
static uint_8t XDATA isbox[256] = isb_data(f1);

static uint_8t XDATA gfm2_sbox[256] = sb_data(f2);
static uint_8t XDATA gfm3_sbox[256] = sb_data(f3);
													   
static uint_8t XDATA gfmul_9[256] = mm_data(f9);
static uint_8t XDATA gfmul_b[256] = mm_data(fb);
static uint_8t XDATA gfmul_d[256] = mm_data(fd);
static uint_8t XDATA gfmul_e[256] = mm_data(fe);
...
void AES_set_encrypt_key(const unsigned char *userKey,
                         const int bits,
                         AES_KEY *key)
{
    aes_set_key( userKey, bits/8, key );
}
...
uint_8t AES_cbc_encrypt(const unsigned char *in, unsigned char *out,
		     const unsigned long length, const AES_KEY *ctx,
		     unsigned char *iv, const int enc)
{
    int n_block = length/16;
	u8 temp[16];
 ...
 int aes_test()
{
	...
	unsigned char iv[20] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
	unsigned char sa_iv[20] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
//	unsigned char temp[16] = {0x00  ,0x01  ,0x02  ,0x03  ,0x04  ,0x05  ,0x06  ,0x07  ,0x08  ,0x09  ,0x0a  ,0x0b  ,0x0c  ,0x0d  ,0x0e  ,0x0f};
	//	unsigned char temp[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
		unsigned char temp[16] = {0xa4, 0x5e, 0x36, 0x87, 0x5a, 0x6f, 0x8c, 0xbe, 0x4e, 0x68, 0x24, 0x41, 0x3c, 0xa1, 0x9d, 0x0e};  

	unsigned char temp2[64]= "This is test ok!This is test ok!This is test ok!This is test ok!";
//   unsigned char temp2[16]= {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a};
...
//	       *********************************************************
	 ...
	 AES_cbc_encrypt(in, (unsigned char*)out, length, 
...
//	       *********************************************************
	...
	AES_set_encrypt_key((unsigned char*)userKey, 128, &key);
	AES_cbc_encrypt(out, (unsigned char*)in, length, 
						&key, (unsigned char*)sa_iv,AES_DECRYPT);
//	aes_dec(out, in, length,userKey, sa_iv);
	in[length] = '\0';	
   	FM_Printf(FM_ERROR,"In: %s\n", in);
    for(i=0;i<64;i++)
	{
//		FM_Printf(FM_ERROR,"%02x ",in[i]);
	}
//	getch();
	FM_Printf(FM_ERROR,"\n\n");
	return 0;
