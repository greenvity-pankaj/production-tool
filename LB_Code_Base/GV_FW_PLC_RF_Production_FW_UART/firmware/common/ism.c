...
//    u32 delta, ntb1;
	...
	EA = 1; // Enable Globle Interrupts //Kiran
...
#if defined(POWERSAVE) || defined(LLP_POWERSAVE)
    while(intStatus.s.hpgpBP ||intStatus.s.PosthpgpBP||intStatus.s.ethFreeCP||intStatus.s.eth_txfifoRdDn||intStatus.s.spiRxDone||intStatus.s.spiTxDone||intStatus.s.newEtherSA||intStatus.s.cpuTxQNonEmpty||intStatus.s.plcBcn2Sent||intStatus.s.plcBcn3Sent||intStatus.s.plcBcnRx || PlcMedInterruptReg.s.zcint|| PlcMedInterruptReg.s.plcTxDoneint || PlcMedInterruptReg.s.earlywakeBP || PlcMedInterruptReg.s.bcnStart) 
#else
...
//        HHAL_ProcessPlcTxDone();
...
#ifdef HYBRII_HPGP
     if(PlcMedInterruptReg.s.zcint && gHpgpHalCB.bcnInitDone
#ifdef FREQ_DETECT	 	
    ...
    if((gHpgpHalCB.gFreqCB.freqDetected == FALSE)  && gHpgpHalCB.bcnInitDone)
    {
       ...
       // if(PlcMedInterruptReg.s.zcint && (linkLayer->mode == LINKL_STA_MODE_CCO))
		...
		if(PlcMedInterruptReg.s.zcint && (gHpgpHalCB.devMode == DEV_MODE_CCO))   // PCCO TODO
#endif         
...
//                 WriteU32Reg(PLC_BPST_REG, ctorl(gbpst)); //  bug fix, let HW handle BPST (TRI)
            }
...
#else
                   if(gHpgpHalCB.bcnmisscnt == MAX_MISS_BCN)
#endif
                   {
...
//						gHpgpHalCB.nwSelected = 0;
						...
						if (gHpgpHalCB.devMode == DEV_MODE_PCCO)
						{
							sCnsm*		   cnsm = (sCnsm *)LINKL_GetCnsm(linkLayer);
				...
				//[YM] - should add else condition to clear bcnmisscnt if missing bcn is not conseqtive happened				
...
#ifdef PS_DEBUG
						if (psDebug)
							FM_Printf(FM_USER, "BCN_START_INT: call datapath_transmitDataPlc(), EarlyWakeBcnCnt=%lu, awdCnt=%bu, scb->bpCnt=%u\n", EarlyWakeBcnCnt, awdCnt, scb->bpCnt); 
#endif
		 				datapath_transmitDataPlc(1);
					}
				 }
 ...
 *
 * $Log: ism.c,v $
 * Revision 1.60  2015/01/02 14:55:35  kiran
 ...
 *
 * Revision 1.59  2014/12/23 09:01:33  ranjan
 * Fixed Zigbee LMAC projects
 *
 * Revision 1.58  2014/12/09 07:09:08  ranjan
 * - multicco feature under MCCO flag
 *
 * Revision 1.57  2014/11/11 14:52:56  ranjan
 ...
 *
 * Revision 1.56  2014/11/06 18:28:25  ranjan
 * -disabling recovery_monitor
 * -removing scheduleframetask for interupt stability
 *
 * Revision 1.55  2014/11/06 14:46:15  prashant
 * Plc rx frame stuck issue fix
 *
 * Revision 1.54  2014/10/29 22:29:09  yiming
 * add some comments to code
 *
 * Revision 1.53  2014/10/28 16:27:42  kiran
 ...
 *
 * Revision 1.52  2014/10/15 10:42:51  ranjan
 * small fixes in um
 *
 * Revision 1.51  2014/10/13 10:23:57  prashant
 * LG-Uart corruption issue fix
 *
 * Revision 1.50  2014/09/30 21:39:43  tri
 * Added LLP PS
 *
 * Revision 1.49  2014/09/25 10:57:41  prashant
