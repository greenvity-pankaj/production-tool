...
#endif
	//volatile sSwFrmDesc uartTxFrmSwDesc;
}
...
typedef struct uartRxControl_s
{
	...
	volatile uint8_t 			*pRxdataBuffer;
	//volatile sCommonRxFrmSwDesc uartRxFrmSwDesc;
	//volatile sSwFrmDesc			*uartRxSwDesc;
	volatile sSwFrmDesc			rxSwDesc;
	volatile  uint8_t 		*pCellAddrRx;
...
void uartRxProc();
u8 GV701x_setUartRxTimeout(uint32_t timeout);
...
extern xdata volatile uint8_t rxBuffer[128];
