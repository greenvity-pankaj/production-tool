/*
* $Id: hal_hpgp.c,v 1.63 2014/09/05 09:28:18 ranjan Exp $
*
...
//extern Load_Config_Data (u8, u8 *);
//extern Program_Config_Data();
...
//#endif
	...
	FM_HexDump(FM_USER, "par", (u8*)pTestParams, sizeof(sPlcSimTxTestParams));
    ...
    while(1)
    {
        ...
        {       
            ...
            {
			//printf(" tmpOffsetByte = %bu \n", tmpOffsetByte); 
			//printf(" remDescLen = %bu \n", remDescLen); 
...
void FREQDET_FreqSetting(u8 frequency)
{
    ...
    {                
        ...
        else
        {
            ...
            WriteU32Reg(PLC_SWBCNPERAVG_REG, ctorl(gHpgpHalCB.curBcnPer)); 
            PLC_MIN_AC_BPLEN  =           0xF3000;
            PLC_MAX_AC_BPLEN  =           0xF6000; 
...
void HHAL_SetSnid(u8 snid)
{
    ...
    {
        /*// FIXME : -- sync with HMAC on the use of Set SNID for STA side
        if(gHpgpHalCB.halStats.RxGoodBcnCnt >= PLC_BCNTST_SYNCTHRES)
        {
            gHpgpHalCB.nwSelected  = 1;
            FM_Printf(FM_MINFO, "HHAL_SetSnid: network Selected, snid = %bu\n", snid);			
        }
        else
        {
        PlcDevCtrlReg.s.snidValid = 0;
        }
              */
...
#endif
#endif	 //defined(HYBRII_HPGP) && defined(HYBRII_ZIGBEE)
*/
#ifdef _LED_DEMO_
...
//    u32 E_CCOBTS;
    ...
    if(gHpgpHalCB.snapShot)
    {
      ...
      // printf("\n bcn per = %lu",BcnPer); 
       ...
       if((BcnPer * 40) > MISSING_BCN_PER_THRESHOLD) //if bcn is missed - multiplied with 40 to read in terms of ms
       {
            gBcnPer = gBcnPer;
       } 
       else
       {
         ...
         //this is just debug purpose to see bcn per
         if((gBcnPer * 40) > 42000000)
         {
            //THIS IS PRINTED JUST TO DEBUG WHEN WE KEEP OVERNIGHT TEST WHETHER WE GET THIS OR NOT
#ifndef MPER            
...
void FREQDET_DetectFrequencyUsingBcn()
{
#ifndef HPGP_HAL_TEST
    sHaLayer *hal = HOMEPLUG_GetHal();
#endif 
