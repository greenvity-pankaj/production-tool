 ...
 *
 * $Id: mac_hal.c,v 1.7 2014/06/10 22:46:43 yiming Exp $
 *
...
void mac_hal_init (void)
{
    ...
    os_create_task(ZIGBEE_TASK_ID_HAL);
#endif
#endif
}
...
#ifdef RTX51_TINY_OS
#ifdef ZIGBEE_HAL_TASK
        os_set_ready(ZIGBEE_TASK_ID_HAL);
#else
...
#ifdef RTX51_TINY_OS
#ifdef ZIGBEE_HAL_TASK
void os_mac_hal_task (void) _task_ ZIGBEE_TASK_ID_HAL
{
    while (TRUE) {
        os_wait1(K_SIG);
        mac_hal_task();
...
#ifdef Z_P_BRIDGE
void mac_hal_zb_pkt_bridge (sCommonRxFrmSwDesc* rx_frame_info_p)
{
    uint8_t       frame_len;
#ifdef BRIDGE_DEBUG
    uint8_t       cp_len;
    ...
    for (i = 0 ; i < rx_frame_info_p->cpCount ; i++) {
        cp_addr_p = CHAL_GetAccessToCP(rx_frame_info_p->cpArr[i]);
...
void mac_hal_promis_ack_table_show (void)
{
    ...
    enable_map = 
       hal_common_bit_field_reg_read(ZIG_PROMIS_ACK_SHORT_SRC_EN);
    printf("Source Short Address Table\n");
    ...
    enable_map = 
       hal_common_bit_field_reg_read(ZIG_PROMIS_ACK_EXT_SRC_EN);
    printf("Source Extended Address Table\n");
    for (index = 0; index < ZIG_PROMIS_ACK_SRC_ENTRY_MAX; index++) {  
       addr = ZigPromisAckSrcExt + (index * sizeof(ext_addr));
    ...
    enable_map = 
       hal_common_bit_field_reg_read(ZIG_PROMIS_ACK_SHORT_DST_EN);
    printf("\nDestination Short Address Table\n");
    ...
    enable_map = 
       hal_common_bit_field_reg_read(ZIG_PROMIS_ACK_EXT_DST_EN);
    printf("Destination Extended Address Table\n");
    for (index = 0; index < ZIG_PROMIS_ACK_DST_ENTRY_MAX; index++) {  
       addr = ZigPromisAckDstExt + (index * sizeof(ext_addr));
       ext_addr.lo_u32 = hal_common_reg_32_read(addr);
       ext_addr.hi_u32 = hal_common_reg_32_read(addr + 4);
    ...
    //printf("\nPromiscuous Ack - State: %bu, Mode: %bu\n", state, mode);
