...
#define HPGP_TIME_DISC_AGING_UNASSOC    15000 // test
...
void SNSM_UpdateAvlnList(sSnsm *snsm, sAvlnInfoRef *avlnInfo)
{
    ...
    for(i = 0; i < AVLN_LIST_MAX; i++)
    {
        if(snsm->avlnInfo[i].valid == TRUE)
        {
            if((memcmp(snsm->avlnInfo[i].nid, avlnInfo->nid, NID_LEN-1) == 0) &&
               ((snsm->avlnInfo[i].nid[NID_LEN-1]&NID_EXTRA_BIT_MASK) == (avlnInfo->nid[NID_LEN-1]&NID_EXTRA_BIT_MASK)))
            {
    ...
    {
        FM_Printf(FM_WARN, "SNSM:AVLN List full\n");
    }
}
    ...
    if((memcmp(staInfo->nid, bcnHdr->nid, NID_LEN) == 0))
    {
            ...
            (bcnHdr->bt == BEACON_TYPE_PROXY))
        {
            ...
            if(staInfo->ccoScb == NULL)
            {
                ...
                if(!staInfo->ccoScb) 
                {
                    FM_Printf(FM_WARN, "SNSM:can't get a scb for CCo.\n");
                }
...
FM_Printf(FM_HINFO, "SNSM:get discBcn sched(%d)\n", *dataptr);
            ...
            // Process other bcn also to sync network
                    ...
                    (bcnHdr->bt == BEACON_TYPE_PROXY))
                {
                    ...
                    if ( HHAL_SyncNet(HOMEPLUG_GetHal(), snsm->bpsto) == STATUS_SUCCESS 
                      && !snsm->netSync )
                    {
...
eStatus SNSM_SendMgmtMsg(sSnsm *snsm, u16 mmType)
{
    ...
    switch(mmType)
    {
        ...
        case EVENT_TYPE_CC_DISCOVER_LIST_CNF:
        {
            ...
            FM_Printf(FM_MMSG, "SNSM:>>>CC_DISCOVER_LIST.CNF\n");
            newEvent->eventHdr.eventClass = EVENT_CLASS_MSG;
...
#ifdef HPGP_HAL
                        /* set PhyClk */
                        //HHAL_SetDevMode(linkl->hal, DEV_MODE_CCO, LINE_MODE_DC);
                        
                        //Device should in STA mode during net discovery     
                        //HHAL_SetDevMode(linkl->hal, DEV_MODE_CCO,gHpgpHalCB.lineMode);
                        ...
                        switch(bcnsrc)
                        {
                            case BCN_SRC_CCO:
                            {
									...
									(gHpgpHalCB.snid != staInfo->snid))
								{
									...
									FM_Printf(FM_MINFO,"switch sync to snid %bu\n", staInfo->snid);
								...
								else if(!snsm->netScan)
								{
                                   ...
                                   FM_Printf(FM_MINFO,"nsm:BCN_SRC_CCO\n");
								    ...
								    //HHAL_SetDevMode(linkl->hal, DEV_MODE_STA, LINE_MODE_DC);
                                    HHAL_SetDevMode(linkl->hal, DEV_MODE_STA, 
                                    				gHpgpHalCB.lineMode);
								    /* start scan for sync */
									HAL_ScanNet(TRUE);
                                    snsm->netScan = TRUE;
...
//                        SNSM_ProcBcn(snsm, event);
                        ...
                        switch(snsm->hoSwitch)
                        {
                            ...
                            case HPGP_HO_SWITCH_STA: //switch to the third STA
                            {
                                ...
                                if(scb == NULL)
                                {
                                    FM_Printf(FM_ERROR, "SNSM:Can't alloc scb for CCo\n");
                                    break;
                                }
