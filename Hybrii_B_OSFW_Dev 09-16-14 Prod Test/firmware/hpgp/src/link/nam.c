...
//FM_Printf(FM_HINFO, "CNAM: accNotif %bu\n",cnam->accNotification);   
...

							{
								...
								Host_SendIndication(HOST_EVENT_TYPE_PEER_ASSOC_IND,
            ...
            case EVENT_TYPE_CC_LEAVE_REQ:
            {
                ...
                if(scb && (scb->namState == STA_NAM_STATE_CONN))
                {
					...
					{	
						...
						Host_SendIndication(HOST_EVENT_PEER_STA_LEAVE, (u8*)&pLeave,
        ...
        switch(event->eventHdr.type)
        {
			...
			{		
				...
				else
				{
				    ...
				    {
				        FM_Printf(FM_ERROR, "CNAM:EallocErr\n");
				    }
...
//				              event->eventHdr.type, scb->tei);
                ...
                { 
                    ...
                    else
                    {
                        ...
                        {
                            FM_Printf(FM_ERROR, "CNAM_ProcEvent: EAllocErr\n");
                        }
                    }
                ...
                {
                    FM_Printf(FM_ERROR, "CNAM:unexpected TEI release timer out\n");
                }
                break;
            }
...
//                    scb->hoTrigger = HO_TRIGGER_TYPE_AUTO;
                ...
                {
                    FM_Printf(FM_WARN, "CNAM:Initiate a HO fail\n");
                }
                break;
            }
...
//    sLinkLayer    *linkl = (sLinkLayer *)HPGPCTRL_GetLayer(HP_LAYER_TYPE_LINK);
    ...
    switch(mmType)
    {
        ...
        case EVENT_TYPE_CC_ASSOC_REQ:
        {
            ...
            if(staInfo->ccoScb == NULL)
            {
                    FM_Printf(FM_ERROR, "SNAM:ccoScb NULL\n");
					break;
            }
					...
					case EVENT_TYPE_CC_BACKUP_APPOINT_REQ:
					{
						...
						if (ccoBackupReq->action == HPGP_CCO_BACKUP_REQ_APPOINT)
						{
							...
							if (staInfo->staCap.fields.backupCcoCap)
							{
								...
								Host_SendIndication(HOST_EVENT_SELECTED_BACKUP_CCO, NULL, 0);
...
FM_Printf(FM_ERROR, "SNAM:Renew the TEI\n");
    ...
    FM_Printf(FM_HINFO,"event MAC Addr:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n",
              ...
              hpgpHdr->macAddr[4], hpgpHdr->macAddr[5]);
                            ...
                            {
                                FM_Printf(FM_ERROR, "SNAM:Assoc failed\n");
						    ...
						    if(!snam->teiRenew) 
                            {
								...
								Host_SendIndication(HOST_EVENT_TYPE_ASSOC_IND,
													(u8*)&assocInd, sizeof(assocInd));
...
// UKE                                AKM_Start(&linkl->akm, LINKL_STA_MODE_STA, 0);    
							...
							else
							{
								...
								Host_SendIndication(HOST_EVENT_TEI_RENEWED, (u8*)&staInfo->tei, 1);
							}
...
eStatus IDENTIFY_sendFrm(u16 mmType, sEvent *reqEvent, sScb *scb)
{
    ...
    status = MUXL_TransmitMgmtMsg(HPGPCTRL_GetLayer(HP_LAYER_TYPE_MUX), event);
    //the event is freed by MUXL if the TX is successful
    if(status == STATUS_FAILURE)
    {
        EVENT_Free(event);
