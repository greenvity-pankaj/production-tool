...
void CRM_Init(sCrm *crm)
{
	...
	while(dstScb)
	{
		STM_FreeTimer(dstScb->teiTimer);
		STM_FreeTimer(dstScb->staTimer);
		dstScb->teiTimer = STM_TIMER_ID_NULL;
		dstScb->staTimer = STM_TIMER_ID_NULL;
		dstScb = CRM_GetNextScb(crm, dstScb);
	}
...
//FM_Printf(FM_ERROR, "CRM: free scb (%d)!\n", currscb->tei); 
        ...
        STM_FreeTimer(currscb->staTimer);
		STM_FreeTimer(currscb->teiTimer);
    ...
    while(currlink && nextlink) 
    {
        ...
        if (scb->tei == nextscb->tei)
        {
            ...
            //reset the scb before placing it to the free queue
            STM_FreeTimer(nextscb->staTimer);
			STM_FreeTimer(nextscb->teiTimer);
...
u8 CRM_FindScbVendorField(u16 vendor_field, u8 *macAddr)
{
	sScb* dstScb = NULL;
    sLinkLayer    *linkl = (sLinkLayer *)HPGPCTRL_GetLayer(HP_LAYER_TYPE_LINK);
    sCrm          *crm = LINKL_GetCrm(linkl);
    ...
    while(dstScb)
    {
		if(dstScb->vendor_field == vendor_field)
		{
...
u8 CRM_AddScbVendorField(u8 *macAddr, u16 vendor_field)
{
	sScb* dstScb = NULL;
    sLinkLayer    *linkl = (sLinkLayer *)HPGPCTRL_GetLayer(HP_LAYER_TYPE_LINK);
    sCrm          *crm = LINKL_GetCrm(linkl);
    ...
    while(dstScb)
    {
		if(memcmp(macAddr, &dstScb->macAddr, MAC_ADDR_LEN) == 0)
		{			
			dstScb->vendor_field = vendor_field;
		 	return STATUS_SUCCESS;
		}
        dstScb = CRM_GetNextScb(crm, dstScb);
    }
...
eStatus CRM_GetMacAddr(u8 tei, u8 *macAddr)
{
	sScb    *scb = NULL;
    sSlist  *bucketlist = NULL;
	sLinkLayer    *linkl = (sLinkLayer *)HPGPCTRL_GetLayer(HP_LAYER_TYPE_LINK);
    ...
    SLIST_For_Each_Entry(sScb, scb, bucketlist, link)
    {
        if (scb->tei == tei)
        {
			memcpy(macAddr,scb->macAddr,MAC_ADDR_LEN);
            return STATUS_SUCCESS;
        }
    }
	//FM_Printf(FM_ERROR, "TEI FAIL %bu\n", tei);			
	return STATUS_FAILURE;
}
 ...
 *
 * $Log: crm.c,v $
 * Revision 1.9  2015/01/02 14:55:36  kiran
 ...
 *
 * Revision 1.8  2014/11/11 14:52:58  ranjan
