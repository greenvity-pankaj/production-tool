...
eStatus UartApp_SendData(u8 port, void* databuf, u16 len);
u8 UartApp_Poll(void);
void UartApp_ProcessEvent(sAppEvent *event);
...
void UartApp_Tx(u8 port, u8* databuf, u16 len)
{
	...
	UartApp_SendData(port, databuf, len);
}
...
void UartApp_Rx(u8 port, u8* databuf, u16 len)
{
	...
	if(port == APP_PORT_PLC)
	{
		...
		if((len - ETH_HDR_SIZE) > MAX_PKT_BUFFSIZE)
			return;
		...
		memcpy(buf, (u8*)(databuf + ETH_HDR_SIZE), (len - ETH_HDR_SIZE));

		/*Transfer Data coming from PLC to UART*/
		UartApp_Tx(APP_PORT_PERIPHERAL, (u8*)buf, (len - ETH_HDR_SIZE));
	...
	{	
		...
		if(len > MAX_PKT_BUFFSIZE - ETH_HDR_SIZE)
			return;

		/*Copy payload after ethernet header*/
		memcpy(&buf[ETH_HDR_SIZE], (u8*)databuf, len);
...
eStatus UartApp_SendData(u8 port, void* databuf, u16 len)
{
    ...
    event->eventHdr.eventClass = APP_EVENT_CLASS_DATA;
...
__CRIT_SECTION_END__
		...
		if(event->eventHdr.eventClass != APP_EVENT_CLASS_DATA)
		{
...
{	
	...
	if(((event->eventHdr.eventClass != APP_EVENT_CLASS_CTRL) && 
		(event->eventHdr.eventClass != APP_EVENT_CLASS_MGMT) ))
	{
		return;
	}
	...
	if(event->eventHdr.eventClass == APP_EVENT_CLASS_CTRL) 		  
	{
		switch(pHostHdr->type)
		{
			case CONTROL_FRM_ID:
			{
				...
				switch(*ctrlevent)
				{
					...
					{						
						...
						evntclass = APP_EVENT_CLASS_CTRL;
			...
			case EVENT_FRM_ID:
			{
				...
				if(pHostEvent->eventClass == APP_EVENT_CLASS_CTRL)
				{
	...
	else if(event->eventHdr.eventClass == APP_EVENT_CLASS_MGMT)
	{
