...
#End Region
    ...
    'Public Sub exceuteTests(ByVal cl As ConnectedClient, ByVal t As HomeScreen.tests, ByVal status As HomeScreen.execState)
    ...
    '    t = status.test
    '    MessageBox.Show(status.test)
    '    MessageBox.Show(status.state)

    '    HomeScreen.dumpMsg(vbNewLine)
    '    HomeScreen.dumpMsg(t.ToString)
    '    HomeScreen.dumpMsg(HomeScreen.status.state.ToString)

    '    Select Case status.test

    '        Case HomeScreen.tests.TEST_ID_PLC_TX

    '            Select Case HomeScreen.status.state
