...
void correctEndieness_sRfTxTestHostParams(sRfTxTestHostParams *pTestParams)
{
	pTestParams->srcShortAddress = le16_to_cpu(pTestParams->srcShortAddress);
	pTestParams->dstShortAddress = le16_to_cpu(pTestParams->dstShortAddress);
	pTestParams->panId= le16_to_cpu(pTestParams->panId);
	
	pTestParams->frameCount = le16_to_cpu(pTestParams->frameCount);
	pTestParams->interFrameDelay = le16_to_cpu(pTestParams->interFrameDelay);
}

void correctEndieness_sRfRxTestHostParams(sRfRxTestHostParams *pTestParams)
{
	pTestParams->srcShortAddress = le16_to_cpu(pTestParams->srcShortAddress);
	pTestParams->dstShortAddress = le16_to_cpu(pTestParams->dstShortAddress);
	pTestParams->panId= le16_to_cpu(pTestParams->panId);
